...
C3.Plugins.Jammanna_MakeIt3D.Instance = class DOMMessagingInstance extends globalThis.ISDKInstanceBase {
	constructor() {
		...
		this.runtime.addEventListener("afteranylayoutstart", ev => {
			//console.log("After starting layout:", properties);
		});
	...
	_tick()
	{
		...
		{
			// let {renderer,scene,camera,objects}=window.MakeIt3DContext.threeJsContext;
			// if(renderer && scene && camera)
			// renderer.render(scene,camera);
			// if(objects)
			// {
			// 	const obj = objects.get("Test");
			// 	const obj2 = this.runtime.objects["Sprite"].getFirstInstance();
			// 	//console.log(this.runtime.viewportHeight, this.runtime.viewportWidth)
			// 	//console.log(this.runtime.layout.width, this.runtime.layout.height)
			// 	console.log(obj2)
			// 	if(obj && obj2)
			// 	{
			// 		const layout = this.runtime.layout;
			// 		const pos = this.c3ToThree(obj2.x, obj2.y, obj2.zElevation, this.runtime.viewportWidth, this.runtime.viewportHeight,0.02);
			// 		obj.position.x = pos.x;
			// 		obj.position.z = -pos.y;
			// 		obj.position.y = pos.z;
			// 		obj.rotation.y = -(obj2.angle+(90*Math.PI/180));
					
			// 	}
			// }
			const left = 0;
			const right = this.runtime.layout.width;
			const top = 0;
			const bottom = this.runtime.layout.height;

			// flip Y (since Three.js Y+ goes up, C3 Y+ goes down)
		//  camera = new THREE.OrthographicCamera(left, right, bottom, top, -1000, 1000);
		// 	camera.position.z = 500;  // look down at scene
	...
	_syncC3ToThreeObject(e)
	{
		...
		const rotY = -(e.c3object.angle+(90*Math.PI/180));
		...
		if(e.syncscale)
		{
			this._setObjectProperty({
				objectId: e.objectId,
				property: "scale",
				value: { x: e.c3object.width*0.02, y:1, z: e.c3object.height*0.02 }
			});
		}
