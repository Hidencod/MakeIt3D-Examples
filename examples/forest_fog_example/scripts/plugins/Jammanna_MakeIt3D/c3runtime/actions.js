...
C3.Plugins.Jammanna_MakeIt3D.Acts =
{
	...
	async LoadCustomModel(objectId, path, path_name, position, rotation, scale, isVisible) {
		...
		if (path.endsWith(".glb") || path.endsWith(".fbx")) {
		 ...
		 if (Array.isArray(path_name)) {
			// If path_name is an array, loop through and load each model
			for (let i = 0; i < path_name.length; i++) {
				if (path_name[i].endsWith(".glb") || path_name[i].endsWith(".fbx"))
				await this._LoadCustomModel(objectId, path_name[i], pos, rot, scl, isVisible);
			}
		} else if (typeof path_name === 'object') {
			// If path_name is an object, handle accordingly
			for (const key in path_name) {
				if (path_name.hasOwnProperty(key)) {
					if (key.endsWith(".glb") || key.endsWith(".fbx"))
					await this._LoadCustomModel(objectId, path_name[key], pos, rot, scl, isVisible);
				}
			}
		 } else if (typeof path_name ==="string")
		 {
			 if (path_name !== "" && (path_name.endsWith(".glb") || path_name.endsWith(".fbx")))
			 await this._LoadCustomModel(objectId, path_name, pos, rot, scl, isVisible);
		 }
	},
	async LoadCustomJsonModel(objectId, path, path_name, position, rotation, scale, isVisible) {
		const pos = Utils.Utils.toVector3(position);      // {x: 1, y: 2, z: 3}
		const rot = Utils.Utils.toVector3(rotation);      // {x: 0.5, y: 0.5, z: 0.5}
		const scl = Utils.Utils.toVector3(scale, 0.5);    // {x: 1, y: 2, z: 0}
		const finalpath = path;
		if (typeof path_name === "string" && path_name.endsWith(".glb"))
			finalpath = path_name;
		await this._LoadCustomJsonModel(
			{objectId, finalpath, position:pos, rotation:rot, scale:scl, visible:isVisible}
		);
	...
	AddSprite(objectId, position, rotation, color, scale,opacity,imageurl, control) {
		const pos = Utils.Utils.toVector3(position, 0);
		const rot = Utils.Utils.toVector3(rotation, 0);
		const scl = Utils.Utils.toVector3(scale, 0);
	...
	SetLightShadowEnabled(lightId,enableshadows)
	{
		this._enableLightShadows(
		{
			objectId:lightId,
			enableShadows:enableshadows
		})
	},
	SetLightShadowResolution(lightId, width, height)
	{
		this._setLightShadowResolution
		(
			{
				objectId: lightId,
				width,
	...
	SetLightShadowBias(lightId, bias)
	{
		this._setShadowBias
		(
			{
	...
	SetLightShadowRadius(lightId,shadowRadius)
	{
		this._setLightShadowRadius(
			{
				objectId: lightId,
				shadowRadius
			}
	...
	SetDirectionalShadowBounds(lightId,left, right, top, bottom)
	{
		this._setDirectionalShadowBounds(
			{
				objectId: lightId,left,right,top,bottom
			}
		)
	},
	SetSpotlightShadowFOV(lightId,fov)
	{
		this._setSpotLightShadowFov(
			{
				objectId: lightId,fov
			}
		)
	},
	SetObjectShadowProperties(objectId,castShadow,recieveShadow)
	{
		this._setObjectShadowProperties(
			{
				objectId,castShadow,recieveShadow
			}
		)
	},
	SetLightShadowRange(objectId,near,far)
	{
		this._setLightShadowRange({
			objectId, near, far
		})
		
	},
	SetMaterialAlpha(objectId,alphaTest,transparent, controls)
	{
		this._setMaterialAlpha
		(
			{
				objectId,
				alphaTest,
	...
	SetTextureFromSprite(c3object, objectId, magfilter,minfilter)
	{

		const instance  = this.runtime.objects[c3object.name].getFirstPickedInstance();
		
		if (!instance) return;
		
		
		this._setTextureFromSprite(
			{
				objectId,
				frameIndex: instance.animationFrame,
				animationName: instance.animationName,
				objectType: c3object.name,
				magfilter,
				minfilter
			}
		)
	},
	async AddSpineObject(objectId, skeltonJsonPath, atlasPath, basePath, skinName, position, rotation, scale, materialType,usepremultipliedAlpha,visible) {
		 position = Utils.Utils.toVector3(position, 0);
		 rotation = Utils.Utils.toVector3(rotation, 0);
		 scale = Utils.Utils.toVector3(scale, 1);
		const  material = ["meshbasic","meshstandard","meshlambert"]
		await this._addSpineObject(
			{
				objectId, skeltonJsonPath, atlasPath, basePath, skinName, position, rotation, scale, materialType: material[materialType], usepremultipliedAlpha, visible
			}
		)
	},
	PlaySpineAnimation(objectId,animationName,loop,trackIndex)
	{
		this._playSpineAnimation(
			{
				objectId, animationName, loop, trackIndex
			}
		)
	},
	SetSpineSkin(objectId, skinName)
	{
		this._setSpineSkin(
			{
				objectId, skinName
	...
	SetMixDuration(objectId,fromAnim,toAnim,duration)
	{
		this._setMixDuration(
		{
			objectId, fromAnim, toAnim, duration
		})
	},
	SetSpineSpeed(objectId, speed)
	{
		this._setSpineSpeed(
			{
				objectId, speed
	...
	StopSpineAnimation(objectId,trackIndex,fadeOutDuration)
	{
		this._stopSpineAnimation(
			{
				objectId, trackIndex, fadeOutDuration
			}
		)
	}
